...
void entry(unsigned long magic, unsigned long addr) {
    ...
    fileSystem_init((uint32_t*)(*((uint32_t*)(mbi->mods_addr))));

    /* Initialize pages */
    page_init();
    extern void loadPageDirectory(unsigned int* pointer);
    extern void enablePaging();
    loadPageDirectory((unsigned int*)page_directory);
    enablePaging();

    /* Initialize the cursors locations for all the terminals*/
    cursor_init();
