...
#if 1
// Page Directory -> 1024 entries that each represent 4 MB
...
typedef union page_directory_entry_t 
{
    ...
    struct
    {
        ...
        uint32_t rw : 1; // read/write (1 for rw, 0 for read only)
        uint32_t us : 1; // User/Supervisor (1 = accesible to all, 0 = supervisor only)
        uint32_t pwt : 1; // Write-through caching enable bit 
        uint32_t pcd : 1; // Cache Disable (1 = disable, 0 = will be cached)
        uint32_t accessed : 1; // A -> 1 = this entry has been read during address translation. Needs to be reset by OS manually
        uint32_t differs : 1; // Differs based on type; either reserved or dirty bit
        uint32_t page_size : 1; // PS -> 0 = page is 4 KB and points to a paging table, 1 = 4MB page
        uint32_t global : 1; // Global page; must be configured in CR4 to enable. We probably don't need
        uint32_t avail : 3; // available for our use; unused
        uint32_t lower_address : 10; // if using page table, PAT is 1 bit and then 9 bits are reserved
        uint32_t upper_address : 10; // bits 32-22 of address
    } __attribute__ ((packed));
} page_directory_entry_t;


// Each entry is also 32-bits and the table is 1024 entries long
typedef union page_table_entry_t 
{
    ...
    struct
    {
        ...
        uint32_t pcd : 1; // Cache Disable (1 = disable, 0 = will be cached)
        uint32_t accessed : 1; // A -> 1 = this entry has been read during address translation. Needs to be reset by OS manually
        uint32_t dirty : 1; // D -> 1 = has been written to, 0 = has not been
          
        uint32_t pat : 1; // PS -> 0 = page is 4 KB and points to a paging table, 1 = 4MB page
        uint32_t global : 1; // Global page; must be configured in CR4 to enable. We probably don't need
        uint32_t avail : 3; // available for our use; unused
        uint32_t address : 20; // bits 31-12 of address
    }__attribute__ ((packed));
} page_table_entry_t;
...
page_table_entry_t vid_page_table[1024] __attribute__((aligned(4096)));
...
// /*Puts the addess into cr3*/
void loadPageDirectory(unsigned int*);
// /*Flips the fifth bit of cr4 and the most significant bit of cr0 to high*/
void enablePaging();
